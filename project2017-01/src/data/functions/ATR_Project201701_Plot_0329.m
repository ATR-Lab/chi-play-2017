...
elseif flag.graphs == 5 % DWT
    for temp_m = 1:eval(sprintf('size(val2(1).cd1_dwt_%s, 2);', temp.ty)) % number of trial
        figure;
        for temp_ch = 1:eval(sprintf('size(val2(1).cd1_dwt_%s{1, 1}, 2);', temp.ty)) % number of channel
            eval(sprintf('subplot(5, 1, %d)', temp_ch));
            for temp_s = 1:size(val2, 2) % number of session
                eval(sprintf('plot(val2(%d).cd1_dwt_%s{1, %d}(:, %d))', temp_s, temp_ty, temp_m, temp_ch))
                if temp_s == 1
                    hold on;
            ...
            eval(sprintf('temp.plot_%s = mean(comb_%s.per.%s_ch%d, 2);', temp.ty, temp.ty, temp.state, temp_ch));
            eval(sprintf('plot(temp.plot_%s, ''k'', ''LineWidth'', 2)', temp.ty));
