...
for temp_s = 1:size(data, 2) % number of sessions
    for temp_m = 1:size(data(temp_s).epoch, 2) % motion
        for temp_ch = 2:size(data(temp_s).epoch{1, temp_m}, 2) % number of channels
            data(temp_s).detrend_f{1, temp_m}(:, temp_ch - 1) = detrend(data(temp_s).epoch{1, temp_m}(:, temp_ch));
        end
    end
end
