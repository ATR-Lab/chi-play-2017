...
for temp_s = 1:size(data, 2)
    for temp_m = 1:size(data(temp_s).bandstop_f, 2)
        ...
        temp = rmfield(temp, {'startPNT', 'endPNT'});
    end
end
    ...
    for temp_s = 1:size(data, 2) % number of sessions
        for temp_m = 1:size(data(1).task_o, 2) % number of motion
            ...
            for temp_ch = 1:size(data(1).task_o{1, temp_m}, 2) % number of channel
                eval(sprintf('comb_%s.timedomain.%s_ch%d(:, %d) = data(%d).task_%s{1, %d}(1:temp.min_%s, 1);', temp.temp_ty, temp.state, temp_ch, temp_s, temp_s, temp.temp_ty, temp_m, temp.temp_ty));
            end
        end
    end
end
temp = rmfield(temp, {'length_bf', 'length_o', 'length_bfo', 'min_bf', 'min_o','min_bfo', 'temp_ty'});
...
if flag.time_anlys == 2
    for temp_m = temp.time_m % motion; 1:eval(sprintf('size(data(%d).task_%s, 2)', temp_s, temp.ty))
        figure;
        for temp_s = temp.time_s % number of session
            subplot(size(temp.time_s, 2), 2, 1 + 2 * (temp_s - min(temp.temp_s)))
            eval(sprintf('plot(data(%d).task_bf{1, %d}(: , %d))', temp_s, temp_m, temp.time_ch));
            subplot(size(temp.time_s, 2), 2, 2 + 2 * (temp_s - min(temp.temp_s)))
            eval(sprintf('plot(data(%d).task_o{1, %d}(: , %d))', temp_s, temp_m, temp.time_ch));
        end
    end
end